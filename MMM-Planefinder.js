...
Module.register("MMM-Planefinder", {
    ...
    createFlightsTable: function() {
        ...
        var headers = ["Flight", "Aircraft", "Route", "Altitude", "Speed", "Heading"];
        ...
        for (var i = 0; i < Math.min(this.flights.length, this.config.maxFlights); i++) {
            ...
            var heading = this.degreesToCompass(flight.heading);
            headingCell.innerHTML = heading;
            headingCell.className = "planefinder-cell";
            ...
            // Create a single cell that spans all columns (now 6 instead of 5)
            var decodedCell = document.createElement("td");
            decodedCell.colSpan = 6;
            ...
            // Middle - Aircraft type
            var aircraftDiv = document.createElement("div");
            aircraftDiv.className = "planefinder-decoded-aircraft";
            var aircraftName = this.getAircraftName(flight.type);
            aircraftDiv.innerHTML = aircraftName || "";
            ...
            decodedContainer.appendChild(aircraftDiv);
    ...
    socketNotificationReceived: function(notification, payload) {
        if (notification === "FLIGHT_DATA_RECEIVED") {
            ...
            Log.info("MMM-Planefinder: Loaded " + Object.keys(this.airlines).length + " airlines, " + Object.keys(this.airports).length + " airports, and " + Object.keys(this.aircraft).length + " aircraft types");
        }
    },
    ...
    extractFlightData: function(aircraft) {
        ...
        return {
            ...
            heading: aircraft.heading || aircraft.track || aircraft.trk || aircraft.hdg || null
        };
    },
